@echo off
ECHO Starting Vectrieve v0.4 (Ollama Edition)...

REM --- Запускаємо "Кухню" (api.py) у новому вікні ---
ECHO Starting Backend API (api.py)...
start "Vectrieve API" cmd /k ".\venv\Scripts\activate.bat && python api.py"
...
start "Vectrieve UI" cmd /k ".\venv\Scripts\activate.bat && streamlit run app.py"
